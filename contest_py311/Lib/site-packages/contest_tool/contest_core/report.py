...
def PRECONDITION(precondition):
    ...
    """
    timestamp = datetime.now().strftime("%H:%M:%S.%f")
    print("\n\n[PRECONDITION-" + timestamp + "]", precondition)
...
def REVIEWID(review_id):
    ...
    if pattern.search(review_id):
        ...
        logging.warning(
            "Invalid character(s) %s detected in REVIEWID '%s' string.", str(invalid_content)[1:-1], review_id
        )


def REVIEW_ID(review_id):
    """
    Deprecated, please use ``REVIEWID``
    :param str review_id: Review tool ID
    """
    print("//" * 100)
    print("'REVIEW_ID' will be deprecated, please replace it with 'REVIEWID'")
    print("//" * 100)
    REVIEWID(review_id)
