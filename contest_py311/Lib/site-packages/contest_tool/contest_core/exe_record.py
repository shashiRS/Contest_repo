...
from contest_core.global_params import _PROJECT_SPECS, _Test_Verdict
...
def save_exe_record():
    ...
    run_options = {
        ...
        "seq_run": _PROJECT_SPECS["seq_run"],
        "filter_logic": _PROJECT_SPECS["filter_logic"],
        "filter": _PROJECT_SPECS["filter"],
        "json_vars": _PROJECT_SPECS["json_vars"],
        "user_vars": _PROJECT_SPECS["user_vars"],
    }
    execution_data = {
    ...
    for exec_path in report_file:
        with open(exec_path, "w", encoding="utf-8") as file:
            ...
            file.write("\n# Run Options")
            file.write("\n# filter_logic and filter not supported in GUI, implementation for future development\n")
...
def process_exec_data(args):
    ...
    # Get random_execution flag, number of loops, run mode seq run and timestamp enabled status
    ...
    args.seq_run = exec_data["seq_run"]
    # filter_logic and filter not supported in GUI, implementation for future development
    # args.filter_logic = exec_data["filter_logic"]
    # filter_tag_values = exec_data["filter"]
    # insert 'tag' to identify as a filter tag
    # filter_tag_values.insert(0, 'tag')
    # args.filter = [filter_tag_values]
    args.json_vars = exec_data["json_vars"]
    args.user_vars = exec_data["user_vars"]
