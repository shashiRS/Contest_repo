...
# import-error disabled: imports are handled by sys.path additions
# import-outside-toplevel disabled: to get updated values we can access at locations as required
# protected-access disabled: the access is required to read and write values
...
_Test_Case_MetaData_Files = []


def set_global_parameter(
    key: str, value: Union[str, bool, float, object, int, list[Any]], overwrite: bool = False
) -> None:
    """
    ...
    Example::
        ...
        from contest_tool.contest_core.global_params import set_global_parameter

        # sets a global parameter of name "num_1" with integer value 10
        set_global_parameter("num_1", 10)
    """
    # importing the relevant variable which shall be aligned across all script to get updated values
    from contest_core.global_params import _GLOBAL_PARAMS

    if not overwrite and key in _GLOBAL_PARAMS:
        raise KeyError(f"Key '{key}' already exists in global parameter. Please use a different one.")
...
def set_local_parameter(
    ...
    Example::
        ...
        from contest_tool.contest_core.global_params import set_local_parameter

        # sets a local parameter of name "flag" with boolean value True
        set_local_parameter("flag", True)
    """
    # importing the relevant variable which shall be aligned across all script to get updated values
    from contest_core.global_params import _GLOBAL_PARAMS, _LOCAL_PARAMS

    if not overwrite and (key in _GLOBAL_PARAMS or key in _LOCAL_PARAMS):
        raise KeyError(f"Key '{key}' already exists in global or local parameter." " Please use a different one.")
    _LOCAL_PARAMS[key] = value


def set_reporting_parameter(name_of_param: str, value_of_param: str) -> None:
    """
    ...
    Example::
        ...
        from contest_tool.contest_core.global_params import set_reporting_parameter
    ...
    """
    # importing the relevant variable which shall be aligned across all script to get updated values
    from contest_core.global_params import _REPORT_PARAMS

    if name_of_param in _REPORT_PARAMS:
        raise KeyError(
            f"Parameter '{name_of_param}' already exists in report parameters." " Please use a different one."
        )
    _REPORT_PARAMS[name_of_param] = value_of_param


def add_meta_data_links_to_reports(files: Union[str, list[str]]) -> None:
    """
    Function to add metadata file(s) in HTML and JSON report of the test case in which this API is called
    ...
    Example::
        ...
        from contest_tool.contest_core.global_params import add_file_hyperlink_to_html_report
    ...
    """
    # importing the same way to get updated values as done in json_report.py & html_report.py
    from contest_core.global_params import _Test_Case_MetaData_Files
    ...
    if isinstance(files, str):
        ...
        if not os.path.exists(files):
            contest_asserts.fail(f"File does not exist: {files}")
        # check the file already exits in list or not
        if files in _Test_Case_MetaData_Files:
            contest_asserts.fail(f"File already added: {files}")
        ...
        if files_not_existing:
            contest_asserts.fail(f"List of files or single file does not exist: {files_not_existing}")
        if files_already_added:
            contest_asserts.fail(f"List of files or single file already added with same name: {files_already_added}")


# Required for implementation
# pylint: disable=invalid-name
def add_file_hyperlink_to_html_report(files: Union[str, list[str]]) -> None:
    """
    ...
    """
    LOG.info(
        "'add_file_hyperlink_to_html_report' API will be deprecated in upcoming ConTest releases. "
...
def get_gui_stop_state() -> bool:
    ...
    Example::
        ...
        from contest_tool.contest_core.global_params import get_gui_stop_state
...
def _get_files_for_report() -> list[str]:
    """
...
def _clear_files_for_report() -> None:
    """
...
def get_reporting_parameter() -> Union[str, dict[str, str]]:
    """
    ...
    Example::
        ...
        from contest_tool.contest_core.global_params import get_reporting_parameter
...
def get_parameter(key: str, default: object = None) -> Union[str, bool, float, object, int, list[Any]]:
    """
    ...
    Example::
        ...
        from contest_tool.contest_core.global_params import get_parameter
    ...
    """
    # importing the relevant variable which shall be aligned across all script to get updated values
    from contest_core.global_params import _LOCAL_PARAMS, _GLOBAL_PARAMS, _REPORT_PARAMS
    ...
    raise KeyError(
        f"Key '{key}' is not found in global, local or report parameter list. Please check if you have created this "
        f"key in global or local setups."
    )


# Required as per logic implementation
# pylint: disable=inconsistent-return-statements
def get_cfg_paths(cfg_path_name: str = "") -> Union[str, dict[str, str]]:
    """
    ...
    Example::
        ...
        from contest_tool.contest_core.global_params import get_cfg_paths
    ...
    contest_asserts.fail(
        f"Config path name: '{cfg_path_name}' is not found! Please check if the entered path name is correct. "
        f"Available config paths: {path_name_list}"
    )


def clear_global_params() -> None:
    """
    .. note::
...
def clear_local_params() -> None:
    """
    .. note::
        JUST TO BE CALLED FROM TEST FRAMEWORK. DON'T CALL THIS IN YOUR SCRIPTS.
    """
    # importing the relevant variable which shall be aligned across all script to get updated values
    from contest_core.global_params import _LOCAL_PARAMS

    _LOCAL_PARAMS.clear()


def clear_reporting_params() -> None:
    """
    .. note::
        JUST TO BE CALLED FROM TEST FRAMEWORK. DON'T CALL THIS IN YOUR SCRIPTS.
    """
...
# Required per execution logic and readability
# pylint: disable=no-else-return
def get_test_verdict(test_name: str = None) -> Union[str, dict[str, str]]:
    """
    ...
    :returns: Requested test case verdict in form of string or dictionary with verdicts of all tests.
        Possible values ``NOT_EXEC, NOT_AVAILABLE, PASS, INCONCLUSIVE, FAIL, ERROR, SKIP, BLOCK, NONE``

    :rtype: str or dictionary

    Example::

        from contest_tool.contest_core.global_params import get_test_verdict
    ...
    """
    # importing the same way to get updated values as done in test_watcher.py
    from contest_core.global_params import _Test_Verdict, _ALL_TESTS_TO_RUN
...
def get_testcase_data(test_name: str) -> Union[str, dict[str, str]]:
    """
    ...
    Example::
        ...
        from contest_tool.contest_core.global_params import get_test_data_info

        # getting test case data of "SWT_MY_TESTv1" test case
        test_data_info = get_testcase_data("SWT_MY_TESTv1")
    """
    # importing the relevant variable which shall be aligned across all script to get updated values
    from contest_core.global_params import _Test_Data_Info
...
def manual_verification(query: str) -> list[str]:
    """
    ...
    Example::
        ...
        from contest_tool.contest_core.global_params import manual_verification
    ...
    _PROJECT_SPECS["gui_object"][0].emit_sig("test_manual_input", [query, inspect.stack()[1][3]])
...
def get_selected_pytests_data_objs():
    ...
    Example::
        ...
        from contest_tool.contest_core.global_params import get_selected_pytests_data_objs
...
# Required for implementation
# pylint: disable=invalid-name
def get_selected_cmm_tests_data_objs() -> list[str]:
    """
    ...
    Example::
        ...
        # importing the relevant function
        from contest_tool.contest_core.global_params import get_selected_cmm_tests_data_objs
...
# Required for implementation
# pylint: disable=invalid-name
def get_selected_capl_tests_data_objs() -> list[str]:
    """
    ...
    Example::
        ...
        # importing the relevant function
        from contest_tool.contest_core.global_params import get_selected_capl_tests_data_objs
...
def get_selected_pytests_folders() -> list[str]:
    """
    ...
    Example::
        ...
        # importing the relevant function
        from contest_tool.contest_core.global_params import get_selected_pytests_folders
...
def get_selected_pytests_names() -> list[str]:
    """
    ...
    Example::
        ...
        # importing the relevant function
        from contest_tool.contest_core.global_params import get_selected_pytests_names
...
def get_specific_pytest_data(test_name: str) -> Union[None, object]:
    """
    ...
    Example::
        ...
        # importing the relevant function
        from contest_tool.contest_core.global_params import get_specific_pytest_data
...
# Required per execution logic
# pylint: disable=import-outside-toplevel
def get_current_test_info() -> object:
    """
    ...
    Example::
        ...
        from contest_tool.contest_core.global_params import get_current_test_info
    ...
    from contest_core.test_watcher import CURRENT_TESTCASE

    test_case_info = CURRENT_TESTCASE[0]
    return test_case_info


def get_user_cli_vars(var_name: str = "") -> Union[Dict[str, Any], Any]:
    """
    Function to get dictionary with user defined variables in cli (--user_vars and/or --json_vars)

    .. note::

        This API will only log a warning and return empty dictionary if user variables requested via ``var_name`` input
        argument were not found or dictionary containing user variables from cli options is empty.


    :param str var_name: User variable name, whose value is required. Default value is ``""``

    :returns: dictionary of user defined variables or value of the requested variable.
    :rtype: dict/any

    Example::

        from contest_tool.contest_core.global_params import get_user_cli_vars

        my_vars= get_user_cli_vars()
    ...
    Example::
        ...
        value_1 = get_user_cli_vars(var_name="var_1")
        print(value_1)
    """
    user_data = {}
    # If no variables were defined in cli, warn user.
    if not global_vars.USER_CLI_VARS:
        LOG.warning("Dictionary with user defined variables from cli is empty.")
    else:
        # If variable name was not provided, return whole dictionary
        if not var_name:
            user_data = global_vars.USER_CLI_VARS
        # If variable name was provided, return corresponding value or "variable not found" if key does not exists.
        else:
            user_data = global_vars.USER_CLI_VARS.get(var_name, "User variable not found.")
            if user_data == "User variable not found.":
                LOG.warning(
                    "Requested user variable '%s' not found in dictionary containing user variables from cli "
                    "options.",
                    var_name,
                )
                user_data = {}
...
def get_user_machine_name() -> str:
    """
    Method to get the machine name
    TODO: implementing code to fetch machine name
    """
    return "NA"


def is_contest_gui_running() -> bool:
    """
    Function to check if ConTest main tool GUI is running or not.

    :return: ``True`` if contest gui is running else ``False``
    :rtype: bool

    Example::

        from contest_tool.contest_core.global_params import is_contest_gui_running

        gui_running = is_contest_gui_running()
        print(f"ConTest GUI running {gui_running}")
    """
    ret_val = False
    if global_vars.__UI_RUNNING:
        ret_val = True
    return ret_val
