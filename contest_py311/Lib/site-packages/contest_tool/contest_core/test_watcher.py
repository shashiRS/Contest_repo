...
from contest_core.global_params import _Test_Verdict, _ALL_TESTS_TO_RUN, _Test_Data_Info
from global_vars import TestVerdicts

# global list to be shared between 'test_watcher.TestCaseInfo' class and
# 'contest_tool.contest_core.report' module functions
...
class TestCaseInfo:
    ...
    @staticmethod
    def get_testcase_exec_count(name: str) -> int:
        """
        Will return current testcase execution count.
        testcase execution count info is parsed from testcase name.
        example: "SWT_DEMO_TEST_PARAMETERIZEDv6-testrun__1(index: 1)"
        testrun__1, defines the number of testcase execution count

        :param str name: The testcase name

        :rtype: int
        :returns: returns testcase execution count
        """
        match = re.search(r"__([0-9]+)", name)
        if match:
...
class TestWatcherRunner:
    ...
    def test_started(
        ...
        testcase = TestCaseInfo(
            name, test_function, verified_ids, automates_id, tags, test_type, index, user_modified_param_test
        )
