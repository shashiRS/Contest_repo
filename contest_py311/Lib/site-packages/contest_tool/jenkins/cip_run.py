...
def parse_args():
    ...
    parser.add_argument("--log", help="The log level to use", type=str, required=False, default="INFO", dest="loglevel")
...
def cleanup_canoe():
    ...
    LOG.info("Close previously running instances of CANoe to make sure we get a responding executable")
    ...
    if platform.system() == "Windows":
        ...
        if not (
            bool(re.match('ERROR: The process "CANoe32.exe" not found.', exec_output))
            or bool(re.match('FEHLER: Der Prozess "CANoe32.exe" wurde nicht gefunden.', exec_output))
            or bool(re.match('SUCCESS: The process "CANoe32.exe" with PID .* has been terminated', exec_output))
            or bool(re.match('ERFOLGREICH: Der Prozess "CANoe32.exe" mit PID .* wurde beendet.', exec_output))
        ):
        ...
        if not (
            bool(re.match('ERROR: The process "CANoe64.exe" not found.', exec_output))
            or bool(re.match('FEHLER: Der Prozess "CANoe64.exe" wurde nicht gefunden.', exec_output))
            or bool(re.match('SUCCESS: The process "CANoe64.exe" with PID .* has been terminated', exec_output))
            or bool(re.match('ERFOLGREICH: Der Prozess "CANoe64.exe" mit PID .* wurde beendet.', exec_output))
        ):
            abort("--> Unexpected output of taskkill.")
...
def parse_config(file, workspace):
    ...
    if not config or "replace_base_location" not in config:
        LOG.info("'replace_base_location' not defined in config. Fallback to github project root directory.")
        replace_base_location = None
    else:
    ...
    if "filter" in stages[CURR_STAGE]:
        ...
        for single_filter in testcase_filter:
            if "type" not in single_filter or "value" not in single_filter:
                abort("Please provide 'type' and 'value' for each filter. Provided values: '%s'.", single_filter)

            LOG.debug("Filter: '%s' - '%s'", single_filter["type"], single_filter["value"])

    # Read out filter logic operator for current stage.
    filter_logic = ""
    if "filter_logic" in stages[CURR_STAGE]:
        filter_logic = stages[CURR_STAGE]["filter_logic"]
        LOG.debug("Filter logic: %s", filter_logic)
    ...
    # If seq-run flag is given, the tests will be executed in sequential order
    seq_run = False
    if "seq_run" in stages[CURR_STAGE]:
        seq_run = stages[CURR_STAGE]["seq_run"]

        LOG.debug("Sequential flag: %s", (bool(seq_run)))
    ...
    # JSON file path to add user vars
    json_vars = ""
    if "json_vars" in stages[CURR_STAGE]:
        json_vars = stages[CURR_STAGE]["json_vars"]
        if "user_vars" not in stages[CURR_STAGE]:
            abort(
                "No variables passed through --user_vars command. Please pass variable(s) through --user_vars "
                "command."
            )

        LOG.debug("JSON file path: %s", json_vars)

    # Read out user vars to be added
    user_vars_list = []
    if "user_vars" in stages[CURR_STAGE]:
        user_vars = stages[CURR_STAGE]["user_vars"]
        for single_user_var in user_vars:
            user_vars_list.append(single_user_var)

        LOG.debug(" ".join(map(str, user_vars_list)))
...
def generate_parameters(
    ...
    :param bool seq_run: The testcase to be executed in sequential order
    ...
    :param str filter_logic: Logic operator to be applied over given filter type and value(s). Default is 'or'
    :param str json_vars: Path to json file
    :param list user_vars_list: List of user variables to be added
    ...
    if base_location:
        base_location = os.path.join(root_path, base_location)
    else:
        base_location = root_path
    contest_config = os.path.join(root_path, contest_config)
    base_location = base_location.replace("\\", "/")
    contest_config = contest_config.replace("\\", "/")
    parameters = f"-r auto -l {base_location} -c {contest_config} " f"--report-dir {report_dir}"

    # Optional setup file
    if setup_file:
        parameters = parameters + f" --setup-file {setup_file}"
    ...
    if testcase_filter:
        ...
        for tag, value in filter_dict.items():
            ...
            parameters = parameters + f" --filter {filter_string_format}"

    # Optional filter_logic field
    if filter_logic:
        parameters = parameters + f" --filter-logic {filter_logic}"

    # Optional no_of_loops fields
    if no_of_loops:
        parameters = parameters + f" -n {no_of_loops}"

    # Optional ext loc fields
    if ext_loc_list:
        ext_loc_format = " ".join(map(str, ext_loc_list))
        parameters = parameters + f" -e {ext_loc_format}"

    # Optional random field
    if random:
        parameters = parameters + " --random"

    # Optional sequential field
    if seq_run:
        parameters = parameters + " --seq-run"

    # Optional rest_port field
    if rest_port:
        parameters = parameters + f" --rest-port {rest_port}"
    ...
    # Optional json_vars field
    if json_vars:
        parameters = parameters + f" --json-vars {json_vars}"

    # Optional user_vars field
    if user_vars_list:
        user_vars_format = " ".join(map(str, user_vars_list))
        parameters = parameters + f" --user-vars {user_vars_format}"
...
def execute_contest(parameters):
    ...
    """
    cmd = f"{sys.executable} {os.path.dirname(__file__)}/../main.py {parameters}"
    LOG.info(f"Running ConTest with the command: '{cmd}'")
