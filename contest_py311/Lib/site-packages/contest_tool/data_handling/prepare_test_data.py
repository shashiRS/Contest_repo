...
class PrepareTestData:
    ...
    def prepare_canoe_tests(self, canoe_cfg, use_ctf, cte_zip, cte_cfg):
        ...
        # if ctf is old version then fetch test module data from canoe cfg itself
            ...
            try:
                ...
                from contest_canoe import canoe
                        ...
                        try:
                            ...
                            # non-gui mode
                                ...
                                self.log(
                                    ...
                                    InfoLevelType.ERR)
                    # fetch the parameterized test case object from variable which is created in parameterized.py
                    # "set_parameterized_test_data" function
                    # test object contains parameterized test case information of 'TESTTAG', 'VERIFIES', 'DETAILS',
                    # 'PRECONDITION', etc...
                    test_obj = test_scripts_objects[test_name].__FUNC_OBJ__
    ...
    def get_py_func_obj_in_script(self, python_script_path):
        ...
        # NOTE: the 'inspect.isfunction' second arg in 'inspect.getmembers' is removed while working on creating
        # contest wheel package. If this arg is kept, all test functions having any decorator will not be picked up
        # to be added in 'dict_of_functions' dictionary for further processing
    ...
    def prepare_test_data_from_cfg(self, configuration=None, filter_values=None, load_canoe=True, setup_file=None):
        ...
        if "CONTEST_EXTERNAL_RELEASE" not in os.environ:
            client_informer.REST_CLIENT_INFORMER.config_info = {
                "base_path": self.current_cfg.base_path,
                "python_tests": self.current_cfg.ptf_test_path,
                "t32_tests": self.current_cfg.cmm_test_path,
            ...
            client_informer.REST_CLIENT_INFORMER.config_info = {
                ...
                "python_tests": self.current_cfg.ptf_test_path,
                "canoe_cfg": self.current_cfg.canoe_cfg_path,
                "report_path": self.current_cfg.report_path,
                "additional_paths": self.current_cfg.additional_paths,
                "no_of_loops": self.current_cfg.num_loops,
                "selected_tests": self.current_cfg.selected_tests}
    ...
    def filter_tests_to_run(self, tests_in_cfg=None, filter_logic=None, filter_tag_values=None,
                            reverse_tests_selection=False):
        """
        ...
        :param string filter_logic: Logic operator to apply in the filter i.e. and, or
        ...
        if filter_tag_values is not None:
            ...
            for python_test in self.data["py_test_data"]:
                if filter_logic == ValidFilterLogicOperators.OR.value:
                    if any(tag in python_test.tag for tag in filter_tag_values):
                        ...
                        tests_found = True
                if filter_logic == ValidFilterLogicOperators.AND.value:
            ...
            if not tests_found:
                raise RuntimeError(
                    f"No tests filtered for given tag list {filter_tag_values} while applying '{filter_logic}' "
                    "logic operator.")
    ...
    def prepare_runner_data(self, randomize, report_dir, standard_setup_funcs, tests_freq, run_mode, external_paths_cli,
                            timestamp, seq_run, filter_logic, filter_tag_values, json_vars, user_vars):
        """
        ...
        :param bool seq_run: Boolean flag for running tests sequentially or non-sequentially in-case of multiple tests
        :param string filter_logic: Logic operator to apply in the filter i.e. and, or
        :param list filter_tag_values: List containing filter tags
        :param str json_vars: user variables from json file
        :param dictionary user_vars: user variables from cli
        """
        ...
        self.test_runner_data["seq_run"] = seq_run
        ...
        self.test_runner_data["filter_logic"] = filter_logic
        self.test_runner_data["filter"] = filter_tag_values
        self.test_runner_data["json_vars"] = json_vars
        self.test_runner_data["user_vars"] = user_vars
