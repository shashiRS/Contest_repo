...
class UimGui(QDialog, Ui_uim_main):
    ...
    def __init__(self, uim_info_disable=False):
        """
        Constructor
        params boolean uim_info_disable: uim info popup will be disabled when the value is True
        """
        super().__init__()
        self.uim_info_disable = uim_info_disable
        self.setupUi(self)
        ...
        for row, (row_key, row_data) in enumerate(data.items()):
            ...
            for col, col_key in enumerate(self.column_headers):
                if col_key == UimTableColumns.SELECT.name:
                    ...
                    doc_button.clicked.connect(lambda _, r=row: self.view_doc(r))
    ...
    def view_doc(self, row):
        """
        Method connected to view documentation button

        param int row: Row number of package whose doc opening is requested
        """
        package_name = self.uim_table_widget.item(row, UimTableColumns.CONTEST_PACKAGES.value).text()
        # removing 'contest_' or 'contest-' from 'package_name' to construct the correct url for lib doc to open in
        # browser
        if "contest" in package_name.replace("contest_", ""):
            lib_name = package_name.replace("contest-", "").replace("-", "_")
        else:
            lib_name = package_name.replace("contest_", "")
        webbrowser.open(ui_helper.TOOL_APIS_LINK.format(lib_name=lib_name))
