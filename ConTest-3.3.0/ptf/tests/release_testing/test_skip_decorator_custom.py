"""
    ...
    :file: test_skip_decorator_custom.py
    :platform: Windows, Linux
    :synopsis:
        Script for tests related to @skip_if decorator usage in auto mode '-r auto' for custom setup test
    :author:
...
class TestSkipIf(unittest.TestCase):
    ...
    paths_dir = {
        'cfg': os.path.join(REPO_ROOT_DIR, 'cfg', 'skip_tests_custom.ini'),
        'main': os.path.join(REPO_ROOT_DIR, 'main.py')
    }
    verdicts_list = ["[PASSED]", "[INCONCLUSIVE]", "[FAILED]", "[SKIPPED]"]
    report_dir_name = "reports_skip_custom"
    ...
    def setUpClass(cls):
        ...
        run_cmd = [
            ...
            '-l', os.path.join(REPO_ROOT_DIR, 'ptf', 'demo_tests', 'skip_tests', 'custom'),
    ...
    def test_skip_on_linux_via_statement(self):
        ...
        if sys.platform == "win32":
            extract_string = "TEST:"
            expected_test_exe_flow_list = [
                extract_string + " Running 'skip_linux_setup'",
                extract_string + " Running on '{}' platform".format(sys.platform),
                extract_string + " Running 'skip_linux_teardown'"
            ]
            expected_verdict = ["[PASSED]"]
        else:
    ...
    def test_skip_on_windows_via_statement(self):
        ...
        if sys.platform == "linux":
            extract_string = "TEST:"
            expected_test_exe_flow_list = [
                extract_string + " Running 'skip_win_setup'",
                extract_string + " Running on '{}' platform".format(sys.platform),
                extract_string + " Running 'skip_win_teardown'"
            ]
            expected_verdict = ["[PASSED]"]
        else:
            extract_string = "Skipping due to reason:"
    ...
    def test_param_test_no_skip(self):
        ...
        extract_string = "TEST:"
        expected_test_exe_flow_list = [
            extract_string + " Running 'cats_setup'",
